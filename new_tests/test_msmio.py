...
Adapted for msmio by Robert McGibbon
September 15, 2012
'''
...
class test_msmio(unittest.TestCase):
    def setUp(self):
        ...
        hdfFile.createCArray("/", "arr_0", tables.Float32Atom(), self.data.shape, filters=msmio.COMPRESSION)
    ...
    def test_load_1(self):
        ...
        TestData = msmio.loadh(self.filename1, 'arr_0')
        npt.assert_array_equal(TestData, self.data)
        
    def test_load_2(self):
        "load using deferred=False"
        TestData = msmio.loadh(self.filename1, deferred=False)['arr_0']
        npt.assert_array_equal(TestData, self.data)
    
    def test_load_2(self):
        "load using deferred=True"
        deferred = msmio.loadh(self.filename1, deferred=True)
        npt.assert_array_equal(deferred['arr_0'], self.data)
        deferred.close()
            
    def test_save(self):
        """Save HDF5 to disk and load it back up"""
        msmio.saveh(self.filename2, self.data)
        TestData = msmio.loadh(self.filename2, 'arr_0')
        npt.assert_array_equal(TestData, self.data)
...
class test_msmio_int(test_msmio):
    ...
    def setUp(self):
        ...
        hdfFile = tables.File(self.filename1, 'a')
        #The filter is the same used to save MSMB2 data
        hdfFile.createCArray("/", "arr_0", tables.Int64Atom(), self.data.shape, filters=msmio.COMPRESSION)
        hdfFile.root.arr_0[:] = self.data[:]
        hdfFile.flush()
        hdfFile.close()
